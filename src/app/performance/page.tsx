  ...
  return (
    <DashboardShell>
      ...
      <section className="mt-7 grid gap-6 xl:grid-cols-[1.3fr_1fr]">
        ...
        <article className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-start justify-between"><div><h2 className="text-lg font-bold text-slate-900">Supervisor feedback</h2><p className="mt-1 text-sm text-slate-500">Assessment from the approved monthly timesheet</p></div><MessageSquareText className="text-blue-600" size={24} /></div><blockquote className="mt-6 rounded-lg border-l-4 border-blue-600 bg-blue-50 p-5 text-sm leading-7 text-slate-700">{data?.supervisorReview.feedback ?? "No supervisor feedback has been recorded for this period."}</blockquote><div className="mt-5 flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-full bg-slate-200 font-bold text-slate-700">{data?.supervisorReview.reviewer.split(" ").map((name) => name[0]).join("") ?? "—"}</div><div><p className="font-semibold text-slate-900">{data?.supervisorReview.reviewer ?? "Not yet reviewed"}</p><p className="text-sm text-slate-500">{data?.supervisorReview.reviewedAt ? `Reviewed ${formatDate(data.supervisorReview.reviewedAt.slice(0, 10))}` : "Awaiting supervisor review"}</p></div></div><div className="mt-6 border-t border-slate-200 pt-5"><p className="text-sm text-slate-500">Supervisor rating</p><div className="mt-2 flex items-center justify-between"><span className="text-3xl font-bold text-slate-900">{data?.supervisorReview.score ?? 0}%</span><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-700">{data?.supervisorReview.status ?? "DRAFT"}</span></div></div></article>
      </section>
